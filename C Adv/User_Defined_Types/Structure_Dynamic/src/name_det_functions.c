 ...
 *person = (person_name_et *)malloc(sizeof(person_name_et));
 ...
 (*person)->second_name = (char *)malloc(len_second * sizeof(char)); 
...
int f=0;
if(!allocate_person(person,strlen(first_arg),strlen(second_arg)))
{
printf("allocate person failed\n");
return FAILURE;
}
if(NULL==strcpy((*person)->first_name,first_arg))
{
printf("strcpy failed\n");
return FAILURE; 
}
if(NULL==strcpy((*person)->second_name,second_arg))
{
printf("strcpy function failed\n");
return FAILURE;
}
return SUCCESS;
}
...
free(arg_person[index]);
arg_person[index]=NULL;
}
}
