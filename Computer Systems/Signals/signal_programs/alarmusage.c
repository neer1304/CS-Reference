...
It demonstrates that there is only one alarm for a process and if the second alarm call is made in a program 
before the first alarm expires then the second alarm resets the value of the alarm and the first alarm does not 
expire. It can be observed by making the sleep time smaller than the alarm expiry time.
