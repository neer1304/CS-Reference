...
After the signal has been sent by another program, OS runs the signal handler for the target program when the 
target program gets the CPU(i.e., is scheduled on the CPU and returning from the kernel mode)
